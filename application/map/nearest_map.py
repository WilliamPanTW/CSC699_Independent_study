...
roll_position = None
take_position = None
number_positions = []  # Store bounding box and text for numbers
...
for detection in result:
    ...
    # Check if the text looks like a number
        ...
        number_positions.append((center, text))  # Store center and text
    ...
    img = cv2.putText(img, text, (top_left[0], top_left[1] - 5), font, 0.5, (0, 255, 0), 2, cv2.LINE_AA)

# Now, find the nearest number to 'Scene' and 'Roll'
def find_nearest_number(position, number_positions):
    if number_positions:  # Ensure there are numbers detected
        return min(number_positions, key=lambda x: calculate_distance(position, x[0]))[1]
    return None
...
if scene_position is not None:
    scene_value = find_nearest_number(scene_position, number_positions)

# Check if roll_position is not None
if roll_position is not None:
    roll_value = find_nearest_number(roll_position, number_positions)

# Print the detected Scene and Roll values
print(f"Scene: {scene_value if scene_value is not None else 'No number detected'}")
print(f"Roll: {roll_value if roll_value is not None else 'No number detected'}")
