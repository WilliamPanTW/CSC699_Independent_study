...
def rename_file():
    global video_path, save_dir, extracted_attributes
    ...
    # Replace placeholders in the filename with values from the extracted attributes
    if extracted_attributes:
        new_filename = new_filename.replace("[roll]", extracted_attributes.get("roll", "N/A"))
        new_filename = new_filename.replace("[scene]", extracted_attributes.get("scene", "N/A"))
        new_filename = new_filename.replace("[take]", extracted_attributes.get("take", "N/A"))
...
filename_label = tk.Label(window, text="Enter new filename (use [roll], [scene], [take] placeholders):", font=("Helvetica", 10))
